...
// Manejar la solicitud OPTIONS para pre-vuelo CORS
...
include_once '../../models/TipoDocumentoIdentidad.php';

// Instanciar la base de datos y el objeto
$database = new Database();
$db = $database->getConnection();
$tipo_documento = new TipoDocumentoIdentidad($db);
...
switch ($method) {
    ...
    case 'POST':
        ...
        if (!empty($data->descripcion)) {
            ...
            echo json_encode(array("mensaje" => "Datos incompletos. No se pudo crear el tipo de documento."));
        }
        break;
    ...
    case 'PUT':
        ...
        if (!empty($data->id) && !empty($data->descripcion) && isset($data->estado)) {
            ...
            http_response_code(400);
            echo json_encode(array("mensaje" => "Datos incompletos. No se pudo actualizar el tipo de documento."));
        }
        break;

    case 'DELETE':
        // Eliminar un registro
        // El frontend ahora envía el ID en el cuerpo de la solicitud DELETE
        $data = json_decode(file_get_contents("php://input"));
        ...
        if (!empty($data->id)) {
            ...
            http_response_code(400);
            echo json_encode(array("mensaje" => "ID de tipo de documento no proporcionado."));
        }
        break;
