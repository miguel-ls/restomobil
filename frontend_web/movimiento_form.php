            ...
            <form id="movimiento-form" action="movimiento_handler.php" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="tipo_documento" name="tipo_documento" placeholder="Ej: Factura, Boleta" value="<?php echo htmlspecialchars($movimiento_data['tipo_documento'] ?? ''); ?>">
                    </div>
                    <div class="form-group">
                ...
                <div class="table-container">
                    <table id="detalle-movimiento-table">
                        ...
                        <tbody id="detalle-movimiento-tbody">
                            <!-- Filas de detalle se generan con JS -->
                        </tbody>
                    </table>
                </div>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('tipo_entidad').addEventListener('change', (e) => {
        ...
        if(tipo === 'C') {
            loadDropdown('#id_entidad', `${API_URL}clientes.php?estado=Activado`, 'id', 'nombres_apellidos');
        } else if (tipo === 'P') {
            loadDropdown('#id_entidad', `${API_URL}proveedores.php?estado=Activado`, 'id', 'nombre');
        } else {
    ...
    // --- INICIALIZACIÓN ---
    await loadDropdown('#codigo_movimiento', `${API_URL}tipo_movimiento.php`, 'id', 'descripcion', initialData?.codigo_movimiento);

    if (initialData && initialData.tipo_entidad) {
        document.getElementById('tipo_entidad').dispatchEvent(new Event('change'));
        // Esperar a que se carguen las entidades y luego seleccionar la correcta
        setTimeout(() => {
             document.getElementById('id_entidad').value = initialData.id_entidad;
        }, 500);
    }
